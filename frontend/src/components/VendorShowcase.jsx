        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {
            vendors.map((vendor, index) => (
              <div key={vendor.id} className="card-vendor animate-fade-in rounded-lg border bg-[rgb(var(--card))] text-[rgb(var(--card-foreground))] shadow-sm" style={{ animationDelay: `${index * 0.1}s` }}>
                <div className='flex flex-col space-y-1.5 p-6 pb-3'>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="relative flex shrink-0 overflow-hidden rounded-full h-12 w-12">
                        <img src={vendor.image} className='aspect-square h-full w-full object-cover' alt={vendor.name} />
                        <div className='flex h-full w-full items-center justify-center rounded-full bg-[rgb(var(--muted))]'>
                          <vendor.icon className="h-6 w-6 text-[rgb(var(--primary))]" />
                        </div>
                      </div>
                ...
                {/* Rating */}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {vendor.specialties.map((specialty) => (
                      <div key={specialty} className="text-xs inline-flex rounded-full border px-2.5 py-0.5 font-semibold transition-all text-[rgb(var(--foreground))]">
                        {specialty}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap gap-2">
                      <button className="inline-flex items-center justify-center gap-2 whitespace-nowrap font-medium transition-all duration-300 text-xs border-2 border-[rgb(var(--primary))] text-[rgb(var(--primary))] bg-transparent hover:bg-[rgb(var(--primary))] hover:text-[rgb(var(--primary-foreground))] hover:shadow-[var(--shadow-soft)] hover:scale-105 active:scale-95 h-9 rounded-lg px-4 py-2 cursor-pointer">
